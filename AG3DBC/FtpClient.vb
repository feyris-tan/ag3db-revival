...
#Region "Upload: File transfer TO ftp server"
        ...
        ''' <summary>
        ''' Copy a local directory to the FTP server
        ''' </summary>
        ''' <param name="sourceDir">Full path of the local directory</param>
        ''' <param name="targetDir">Target directory on the FTP server</param>
        ''' <returns>Exception if an error occured, Nothing if no errors occured</returns>
        Public Function UploadDirectory(ByVal sourceDir As String, ByVal targetDir As String) As Exception

            If Not Directory.Exists(sourceDir) Then
                Return New Exception("Source directory does not exist.")
            End If

            Dim oFile As FileInfo = Nothing
            Dim oDir As DirectoryInfo = Nothing

            Try

                Dim root As New DirectoryInfo(sourceDir)
                Dim ex As Exception

                FtpCreateDirectory(targetDir)

                For Each oFile In root.GetFiles()
                    Upload(oFile, targetDir & "/" & oFile.Name)
                Next

                For Each oDir In root.GetDirectories()

                    ex = UploadDirectory(oDir.FullName, targetDir & "/" & oDir.Name)
        ...
        Public Function Upload(ByVal fi As FileInfo, Optional ByVal targetFilename As String = "") As Boolean
            ...
            Dim t As New Threading.Thread(AddressOf DataRate)
            t.Start()
                ...
                Try
                    ...
                    Using rs As Stream = ftp.GetRequestStream
                        Do
                            ...
                            Me._bytesTransfered += dataRead
                            Me._tempBytesTransfered += dataRead
                            Me._globalBytesTransfered += dataRead
        ...
        End Function
#End Region
        Private _tempBytesTransfered As Long = 0

        Private Sub DataRate()

            While True
                Me._transferRate = _tempBytesTransfered
                _tempBytesTransfered = 0
...
#Region "Download: File transfer FROM ftp server"
        ...
        Public Function DownloadDirectory(ByVal serverSourceDir As String, ByVal localTargetDir As String, Optional ByVal overwrite As Boolean = True) As Exception
            ...
            Try
                ...
                For Each fs As FTPfileInfo In Me.ListDirectoryDetail(serverSourceDir)

                    If fs.NameOnly <> "." AndAlso fs.NameOnly <> ".." Then

                        If fs.FileType = FTPfileInfo.DirectoryEntryTypes.File Then
                            Download(serverSourceDir & "/" & fs.Filename, localTargetDir & "\" & fs.Filename, overwrite)
                        Else

                            ex = DownloadDirectory(serverSourceDir & "/" & fs.Filename, localTargetDir & "\" & fs.Filename)
                    ...
                    Using fs As FileStream = targetFI.OpenWrite
                        Try
                            ...
                            Do
                                ...
                                Me._bytesTransfered += read
                                Me._tempBytesTransfered += read
                                Me._globalBytesTransfered += read
...
#Region "Other functions: Delete rename etc."
        ...
        Public Function FtpDirectoryExists(ByVal path As String) As Boolean
            ...
            If Len(path) Then
                ...
                Try
                    'Dim ftp As Net.FtpWebRequest = GetRequest(GetDirectory(path))
                    ''Set request to do simple list
                    'ftp.Method = "CWD"
                    ...
                    Dim fs As FTPdirectory = ListDirectoryDetail(path)
...
#Region "Properties"
        ...
        Public Property CurrentDirectory() As String
            ...
            End Set
        End Property
        Private _bytesTransfered As Long = 0
        Private _globalBytesTransfered As Long = 0

        Public ReadOnly Property BytesTransfered() As Long

            Get
                Return Me._bytesTransfered
            End Get

        End Property

        Public ReadOnly Property GlobalBytesTransfered() As Long

            Get
                Return Me._globalBytesTransfered
            End Get

        End Property

        Public Sub ResetGlobalBytesTransfered()
            Me._globalBytesTransfered = 0
        End Sub

        Private _transferRate As Long = 0

        Public ReadOnly Property TransferRate() As Long
            Get
                Return Me._transferRate
            End Get
        End Property
